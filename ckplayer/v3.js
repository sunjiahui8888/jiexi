...
var playtime = 0;
var vodhtml = '<div class="player"><div class="play"></div><div class="vod"><video class="video"></video><video class="video" style="display:none"></video></div><div class="cmd"><div class="left tvp_button tvp_play"><button type="button" title="播放/暂停"><span class="tvp_btn_value">播放</span></button></div><div class="centent tvp_time_rail"><span class="tvp_time_total"><span class="tvp_time_loaded" style="width:0"></span></span> <span class="tvp_time_panel"><span class="time1">00:00</span>/<span class="time2">00:00</span></span></div><div class="right tvp_button tvp_fullscreen_button"><button type="button" title="切换全屏"><span class="tvp_quan">全屏</span></button></div><span class="tvp_time_handel_hint" style="display:none"></span></div></div>';
var ydiskvod = {
...
function error() {
	if (vipjiexi.isiPad) {
		if ($('.video')[playerNum].error != null && typeof($('.video')[playerNum].error.code) != undefined && $('.video')[playerNum].error.code == 4) {
			play_up()
		}
	} else {
...
function player(up) {
	...
	var autoplay = vipjiexi.play == 1 ? 'autoplay="autoplay"' : '';
	$.post("/api/v3.php", {
		"url": vipjiexi.url,
		"up": up
	}, function(data) {
		if (data['msg'] == 'ok' && data['url']) {
			if (data['ext'] == 'link') {
				...
				$('#a1').html(vodhtml);
				vipjiexi.isiPad = true;
				ydiskvod.init(data['url']);
				var h = parseFloat($('body').css('height'));
				$('.video').css('height', (h - 40));
				setInterval('error()', 1000);
			} else if (vipjiexi.isiPad || data['ext'] == 'h5') {
				$('#a1').html('<video class="video" src="' + data['url'] + '" controls="controls" ' + autoplay + ' style="margin-bottom:0px;"></video>');
				vipjiexi.isiPad = true;
				setInterval('error()', 1000);
			} else {
				if (data['ext'] == 'm3u8' || data['ext'] == 'm3u8_list') {
					var flashvars = {
						f: vipjiexi.webpath + 'ckplayer/m3u8.swf',
						a: data['url'],
						c: 0,
						s: 4,
						lv: 0,
				...
				CKobject.embedSWF(vipjiexi.webpath + 'ckplayer/ckplayer.swf', 'a1', 'ckplayer_a1', '100%', '100%', flashvars, params);
			}
			$('#loading').hide();
			$('#a1').show();
		} else {
